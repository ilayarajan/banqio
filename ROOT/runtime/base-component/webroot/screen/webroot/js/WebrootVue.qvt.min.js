moqui.urlExtensions={js:"qjs",vue:"qvue",vuet:"qvt"};window.define||(window.define=function(a,b,c){moqui.isString(a)||(c=b,b=a);moqui.isArray(b)||(c=b);return moqui.isFunction(c)?c():c});Vue.filter("decodeHtml",moqui.htmlDecode);Vue.filter("format",moqui.format);moqui.getQuasarColor=function(a){return"success"===a?"positive":"danger"===a?"negative":a};moqui.notifyOpts={timeout:1500,type:"positive"};moqui.notifyOptsInfo={timeout:5E3,type:"info"};moqui.notifyOptsError={timeout:15E3,type:"negative"};
...
c.indexOf("abort")&&(moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsError,{message:"Could not connect to server"})),moqui.webrootVue.addNotify("Could not connect to server","negative")):f||(a="Error: "+c+" ("+b+")",moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsError,{message:a})),moqui.webrootVue.addNotify(a,"negative"))};
...
moqui.loadComponent=function(a,b,c){var d=moqui.urlExtensions.js,e=moqui.urlExtensions.vue,f=moqui.urlExtensions.vuet,g,l,k,h;if("string"===typeof a){var m=a.indexOf("?");0<m?(g=a.slice(0,m),k=a.slice(m+1)):g=a}else g=a.path,l=a.extraPath,k=a.search,h=a.bodyParameters;console.log("Load Component "+JSON.stringify(a)+" Window Width "+window.innerWidth+" Quasar Platform: "+JSON.stringify(Quasar.Platform.is)+" search: "+k);(600>=window.innerWidth||Quasar.Platform.is.mobile)&&(!k||0>k.indexOf("_uiType"))&&
(k=(k||"")+"\x26_uiType\x3dmobile");if(m=moqui.componentCache.get(g))console.info("found cached component for path "+g+": "+JSON.stringify(m)),b(m);else{var n=g;a.renderModes&&0<=a.renderModes.indexOf(e)&&(n+="."+e);if(n.slice(-e.length)===e)return console.info("loadComponent vue "+n+(c?" id "+c:"")),a={type:"GET",url:n,error:moqui.handleLoadError,success:function(a,c,d){205===d.status?(a=d.getResponseHeader("X-Redirect-To"),moqui.webrootVue.setUrl(a)):(a||b(moqui.NotFound),c=0<=d.getResponseHeader("Cache-Control").indexOf("max-age"),
moqui.isString(a)&&0<a.length?(a=httpVueLoader.parse(a,n.substr(0,n.lastIndexOf("/")+1)),c&&moqui.componentCache.put(g,a),b(a)):b(moqui.NotFound))}},h&&!$.isEmptyObject(h)&&(a.type="POST",a.data=h),$.ajax(a);var p=g.slice(-d.length)===d;!p&&a.renderModes&&0<=a.renderModes.indexOf(d)&&(n+="."+d,p=!0);p||(n+="."+f);l&&0<l.length&&(n+="/"+l);k&&0<k.length&&(n+="?"+k);console.info("loadComponent "+n+(c?" id "+c:""));a={type:"GET",url:n,error:moqui.handleLoadError,success:function(a,e,h){if(205===h.status)a=
...
Vue.component("m-container-dialog",{name:"mContainerDialog",props:{id:String,color:String,buttonText:String,buttonClass:String,title:String,width:{type:String},openDialog:{type:Boolean,"default":!1}},data:function(){return{isShown:!1}},template:'\x3cspan\x3e\x3cspan @click\x3d"show()"\x3e\x3cslot name\x3d"button"\x3e\x3cq-btn dense outline no-caps icon\x3d"open_in_new" :label\x3d"buttonText" :color\x3d"color" :class\x3d"buttonClass"\x3e\x3c/q-btn\x3e\x3c/slot\x3e\x3c/span\x3e\x3cm-dialog v-model\x3d"isShown" :id\x3d"id" :title\x3d"title" :color\x3d"color" :width\x3d"width"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/m-dialog\x3e\x3c/span\x3e',methods:{show:function(){this.isShown=
...
Vue.component("m-form",{name:"mForm",props:{fieldsInitial:Object,action:{type:String,required:!0},method:{type:String,"default":"POST"},submitMessage:String,submitReloadId:String,submitHideId:String,focusField:String,noValidate:Boolean},data:function(){return{fields:Object.assign({},this.fieldsInitial),fieldsChanged:{},buttonClicked:null}},template:'\x3cq-form ref\x3d"qForm" @submit.prevent\x3d"submitForm" @reset.prevent\x3d"resetForm"\x3e\x3cslot v-bind:fields\x3d"fields"\x3e\x3c/slot\x3e\x3c/q-form\x3e',methods:{submitForm:function(){if(this.noValidate)this.submitGo();
else{var a=$(this.$el),b=this;this.$refs.qForm.validate().then(function(c){if(c)b.submitGo();else{var d=a.find("div.has-error input, div.has-error select, div.has-error textarea").first();if(d.length)if(c=d.parents("div.panel-collapse.collapse").last(),c.length)if(c.hasClass("in"))d.focus();else if(c=c.prevAll("div.panel-heading").last(),c.length){var e=c.find('a[data-toggle\x3d"collapse"]').first();e.length?e.click():c.click();setTimeout(function(){d.focus()},250)}else d.focus();else d.focus()}})}},
resetForm:function(){this.fields=Object.assign({},this.fieldsInitial);this.fieldsChanged={}},submitGo:function(){var a=$(this.$el),b=null,c=null,d=$(this.buttonClicked||document.activeElement);d.length&&a.has(d)&&d.is('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]')&&(d.is("[name]")&&(b=d.attr("name"),c=d.val()),d.prop("disabled",!0),setTimeout(function(){d.prop("disabled",!1)},3E3));var e=Object.keys(this.fields).length?new FormData:new FormData(this.$refs.qForm.$el);
$.each(this.fields,function(a,b){b&&e.set(a,b)});for(a=e.entries()[Symbol.iterator]();;){var f=a.next();if(f.done)break;var g=f.value,f=g[0],g=g[1];if(moqui.isString(g)&&g.startsWith("__"))e["delete"](f)}e.set("moquiSessionToken",this.$root.moquiSessionToken);b&&e.set(b,c);this.$root.loading++;$.ajax({type:this.method,url:this.$root.appRootPath+this.action,data:e,contentType:!1,processData:!1,headers:{Accept:"application/json"},error:moqui.handleLoadError,success:this.handleResponse})},handleResponse:function(a){this.$root.loading--;
...
Vue.component("m-form-link",{name:"mFormLink",props:{fieldsInitial:Object,action:{type:String,required:!0},focusField:String,noValidate:Boolean,bodyParameterNames:Array},data:function(){return{fields:Object.assign({},this.fieldsInitial)}},template:'\x3cq-form ref\x3d"qForm" @submit.prevent\x3d"submitForm" @reset.prevent\x3d"resetForm"\x3e\x3cslot :clearForm\x3d"clearForm" :fields\x3d"fields"\x3e\x3c/slot\x3e\x3c/q-form\x3e',methods:{submitForm:function(){if(this.noValidate)this.submitGo();else{var a=
this;this.$refs.qForm.validate().then(function(b){b&&a.submitGo()})}},submitGo:function(){var a=null,b=null,c=$(document.activeElement);c.length&&c.is('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]')&&(c.is("[name]")&&(a=c.attr("name"),b=c.val()),c.prop("disabled",!0),setTimeout(function(){c.prop("disabled",!1)},3E3));var d=Object.keys(this.fields).length?new FormData:new FormData(this.$refs.qForm.$el);$.each(this.fields,function(a,b){!b||moqui.isString(b)&&b.startsWith("__")||
d.set(a,b)});for(var e=[],f=[],g="",l=null,k=d.entries()[Symbol.iterator]();;){var h=k.next();if(h.done)break;var m=h.value,h=m[0],m=m[1];0===m.trim().length||"moquiSessionToken"===h||"moquiFormName"===h||0<h.indexOf("[]")||(0<h.indexOf("_op")||0<h.indexOf("_not")||0<h.indexOf("_ic")?e.push({name:h,value:m}):(f.push(h),this.bodyParameterNames&&0<=this.bodyParameterNames.indexOf(h)?(l||(l={}),l[h]=m):(0<g.length&&(g+="\x26"),g+=encodeURIComponent(h)+"\x3d"+encodeURIComponent(m))))}for(k=0;k<e.length;k++)h=
e[k],m=h.name.substring(0,h.name.indexOf("_")),0<=f.indexOf(m)&&(0<g.length&&(g+="\x26"),g+=encodeURIComponent(h.name)+"\x3d"+encodeURIComponent(h.value));a&&b&&b.trim().length&&(0<g.length&&(g+="\x26"),g+=encodeURIComponent(a)+"\x3d"+encodeURIComponent(b));a=this.action;a=0<a.indexOf("?")?a+"\x26"+g:a+"?"+g;this.$root.setUrl(a,l)},resetForm:function(){this.fields=Object.assign({},this.fieldsInitial)},clearForm:function(){this.fields={}}},mounted:function(){var a=$(this.$el);this.focusField&&0<this.focusField.length&&
a.find("[name\x3d"+this.focusField+"]").addClass("default-focus").focus()}});
...
this.value&&this.value.length&&moqui.isString(this.value)&&this.populateFromUrl({term:this.value}):this.populateFromUrl())}this.multiple||this.allowEmpty||this.value&&this.value.length||!this.options||!this.options.length||this.$emit("input",this.options[0].value)},watch:{curOptionsFoo:function(a){this.value&&this.value.length&&(this.lastVal=this.value)}}});
Vue.component("m-text-line",{name:"mTextLine",props:{value:String,type:String,id:String,name:String,size:String,fields:{type:Object},label:String,tooltip:String,disable:Boolean,defaultUrl:String,defaultParameters:Object,dependsOn:Object,dependsOptional:Boolean,defaultLoadInit:Boolean,rules:Array},data:function(){return{loading:!1}},template:'\x3cq-input dense outlined stack-label :label\x3d"label" v-bind:value\x3d"value" v-on:input\x3d"$emit(\'input\', $event)" :type\x3d"type" :id\x3d"id" :name\x3d"name" :size\x3d"size" :loading\x3d"loading" lazy-rules :rules\x3d"rules" :disable\x3d"disable"\x3e\x3cq-tooltip v-if\x3d"tooltip"\x3e{{tooltip}}\x3c/q-tooltip\x3e\x3c/q-input\x3e',
...
moqui.webrootVue=new Vue({el:"#apps-root",data:{basePath:"",linkBasePath:"",currentPathList:[],extraPathList:[],activeSubscreens:[],currentParameters:{},bodyParameters:null,navMenuList:[],navHistoryList:[],navPlugins:[],notifyHistoryList:[],lastNavTime:Date.now(),loading:0,currentLoadRequest:null,activeContainers:{},moquiSessionToken:"",appHost:"",appRootPath:"",userId:"",locale:"en",notificationClient:null,qzVue:null,leftOpen:!1},methods:{setUrl:function(a,b,c){this.currentLoadRequest&&(console.log("Aborting current page load currentLinkUrl "+
this.currentLinkUrl+" url "+a),this.currentLoadRequest.abort(),this.currentLoadRequest=null,this.loading=0);this.bodyParameters=b;a=this.getLinkPath(a);if(this.currentLinkUrl===a&&a!==this.linkBasePath)this.reloadSubscreens(),c&&this.callOnComplete(c,this.currentPath);else{var d=this.currentPath;b=moqui.parseHref(a);this.extraPathList=[];this.currentSearch=b.search;this.currentPath=b.path;b=this.currentSearch;if((b=this.currentPath+(0<b.length?"?"+b:""))&&0!==b.length){console.info("Current URL changing to "+
b);this.lastNavTime=Date.now();this.activeContainers={};var e=this;b=this.appRootPath&&this.appRootPath.length&&0===b.indexOf(this.appRootPath)?this.appRootPath+"/menuData"+b.slice(this.appRootPath.length):"/menuData"+b;$.ajax({type:"GET",url:b,dataType:"text",error:moqui.handleAjaxError,success:function(a){var b=null;try{b=JSON.parse(a)}catch(l){console.info("Error parson menu list JSON: "+l)}b&&moqui.isArray(b)&&(e.navMenuList=b,c&&e.callOnComplete(c,d))}});window.history.pushState(null,this.ScreenTitle,
a)}}},callOnComplete:function(a,b){if(a){var c=this.getRoute();b&&(c.redirectedFrom=b);a(c)}},getRoute:function(){return{name:this.currentPathList[this.currentPathList.length-1],meta:{},path:this.currentPath,hash:"",query:this.currentParameters,params:this.bodyParameters||{},fullPath:this.currentLinkUrl,matched:[]}},setParameters:function(a){a&&(this.$root.currentParameters=$.extend({},this.$root.currentParameters,a));this.$root.reloadSubscreens()},addSubscreen:function(a){a.pathIndex=this.activeSubscreens.length;
...
data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"QUASAR_DARK",preferenceValue:this.$q.dark.isActive?"true":"false"}})},toggleLeftOpen:function(){this.leftOpen=!this.leftOpen;$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"QUASAR_LEFT_OPEN",preferenceValue:this.leftOpen?"true":"false"}})},stopProp:function(a){a.stopPropagation()},getNavHref:function(a){a||(a=this.navMenuList.length-
1);a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+"/"+a.extraPathList.join("/"),c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams||a.path},getLinkPath:function(a){this.appRootPath&&this.appRootPath.length&&0!==a.indexOf(this.appRootPath)&&(a=this.appRootPath+a);var b=a.split("/"),c=this.appRootPath?2:1;b.length>c&&(b[c]=this.linkBasePath.slice(1),a=b.join("/"));return a},getQuasarColor:function(a){return moqui.getQuasarColor(a)}},
watch:{navMenuList:function(a){if(0<a.length){var b=a[a.length-1];a=1<a.length?a[a.length-2]:null;b.extraPathList&&(this.extraPathList=b.extraPathList);var c=this.basePathSize,d=b.path.split("/").slice(c+1);console.info("nav updated fullPath "+JSON.stringify(d)+" currentPathList "+JSON.stringify(this.currentPathList)+" cur.path "+b.path+" basePathSize "+c);this.currentPathList=d;this.reloadSubscreens();a=(a?a.title+" - ":"")+b.title;var c=b.pathWithParams,e=c.indexOf("?");if(0<e){for(var d=["pageIndex",
"orderBySelect","orderByField","moquiSessionToken"],f=c.substring(e+1).split("\x26"),c=c.substring(0,e),e=0,g="",l=0;l<f.length;l++){var k=f[l],c=-1===c.indexOf("?")?c+"?":c+"\x26",c=c+k;if(!(3<e)){var h=k.indexOf("\x3d");if(0<h){var m=k.substring(0,h),k=k.substring(h+1);0<m.indexOf("_op")||0<m.indexOf("_not")||0<m.indexOf("_ic")||0<d.indexOf(m)||m===k||(0<g.length&&(g+=", "),g+=decodeURIComponent(k),e++)}}}0<g.length&&(70<g.length&&(g=g.substring(0,70)+"..."),a=a+" ("+g+")")}d=this.navHistoryList;
for(f=0;f<d.length;)d[f].pathWithParams===c?d.splice(f,1):f++;for(d.unshift({title:a,pathWithParams:c,image:b.image,imageType:b.imageType});25<d.length;)d.pop();document.title=a}},currentPathList:function(a){(a=a[a.length-1])&&$(this.$el).removeClass().addClass(a)}},computed:{currentPath:{get:function(){var a=this.currentPathList,b=this.extraPathList;return this.basePath+(a&&0<a.length?"/"+a.join("/"):"")+(b&&0<b.length?"/"+b.join("/"):"")},set:function(a){a&&0!==a.length?("/"===a.slice(a.length-
1)&&(a=a.slice(0,a.length-1)),0===a.indexOf(this.linkBasePath)?a=a.slice(this.linkBasePath.length+1):0===a.indexOf(this.basePath)&&(a=a.slice(this.basePath.length+1)),this.currentPathList=a.split("/")):this.currentPathList=[]}},currentLinkPath:function(){var a=this.currentPathList,b=this.extraPathList;return this.linkBasePath+(a&&0<a.length?"/"+a.join("/"):"")+(b&&0<b.length?"/"+b.join("/"):"")},currentSearch:{get:function(){return moqui.objToSearch(this.currentParameters)},set:function(a){this.currentParameters=
moqui.searchToObj(a)}},currentLinkUrl:function(){var a=this.currentSearch;return this.currentLinkPath+(0<a.length?"?"+a:"")},basePathSize:function(){return this.basePath.split("/").length-this.appRootPath.split("/").length},ScreenTitle:function(){return 0<this.navMenuList.length?this.navMenuList[this.navMenuList.length-1].title:""},documentMenuList:function(){for(var a=[],b=0;b<this.navMenuList.length;b++){var c=this.navMenuList[b].screenDocList;c&&c.length&&c.forEach(function(b){a.push(b)})}return a}},
created:function(){this.moquiSessionToken=$("#confMoquiSessionToken").val();this.appHost=$("#confAppHost").val();this.appRootPath=$("#confAppRootPath").val();this.basePath=$("#confBasePath").val();this.linkBasePath=$("#confLinkBasePath").val();this.userId=$("#confUserId").val();this.locale=$("#confLocale").val();moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);this.leftOpen="true"===$("#confLeftOpen").val();var a=$("#confDarkMode").val();this.$q.dark.set("true"===a);this.notificationClient=
